...
rtm_type = "ext-rtm"
sim_name = "cond-no-mute-v-$(rtm_type)-$(background_type)"
...
for i = 1:n_tot_sample
	...
	if rtm_type == "ext-rtm"
		grad_train[:,:,:,i] = permutedims(JLD2.jldopen(joinpath(joinpath(plotsdir(), "gen_$(rtm_type)-$(background_type)"), savename(misc_dict, "jld2"; digits=6)), "r")["rtm"][keep_offset_idx, :, :], [2, 3, 1])
	else
		load_grad = JLD2.jldopen(joinpath(joinpath(plotsdir(), "gen_rtm-$(background_type)"), savename(misc_dict, "jld2"; digits=6)), "r")["rtm"]
...
if background_type == "1d-special"
	global m0_train = deepcopy(m_train)
elseif background_type == "2d-good"
	global m0_train = deepcopy(m_train)
	for i = 1:size(m_train)[end]
        global m0_train[:,:,1,i] = 1f0./Float32.(imfilter(1f0./m_train[:,:,1,i], Kernel.gaussian(10)))
    end
end
...
for e=1:n_epochs # epoch loop
    ...
    if(mod(e,plot_every)==0) 
	    ...
	    for (test_x, test_y, test_x0, file_str) in [[X_train,Y_train, X0_train, "train"], [X_test, Y_test, X0_test, "test"]]
		    ...
		    for (i,ind) in enumerate((2:div(all_sampls,3):all_sampls)[1:plots_len])
				...
				tight_layout()
			  fig_name = @strdict background_type chan_obs noise_lev_x noise_lev_init n_train e offset_start offset_end n_offsets keep_offset_num
	...
	if(mod(e,save_every)==0) 
		...
		save_dict = @strdict background_type chan_obs unet_lev unet_model n_train e noise_lev_x noise_lev_init lr n_hidden L K Params loss logdet_train l2_cm ssim loss_test logdet_test l2_cm_test ssim_test batch_size offset_start offset_end n_offsets keep_offset_num; 
		@tagsave(
			joinpath(plotsdir("savednet_compass"), savename(save_dict, "bson"; digits=6)),
			save_dict;
			safe=true
		);
	end
